...
const WORD_o =
{
  ...
  open_re:  new RegExp( `${C_o.tag_o.ATAG_s}([${C_o.tag_o.REGEXP_s}]+?)?${C_o.tag_o.PTAG_s}`, 'g' ),
  close_re: new RegExp( `${C_o.tag_o.PTAG_s}${C_o.tag_o.ATAG_s}`, 'g' ),
  term_re:  new RegExp( `${C_o.tag_o.ATAG_s}([${C_o.tag_o.REGEXP_s}]+?)?${C_o.tag_o.PTAG_s}([\\s\\S]*?)${C_o.tag_o.PTAG_s}${C_o.tag_o.ATAG_s}`, 'g' ),
...
module.exports =
{
  ...
  )
  {
    return input_s
      ...
      .replace( WORD_o.open_re, `<${C_o.tag_o.MARK_s} ${C_o.tag_o.DATA_s}="$1">` )
      .replace( WORD_o.close_re, `</${C_o.tag_o.MARK_s}>` )
  },
