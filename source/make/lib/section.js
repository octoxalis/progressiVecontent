...
const SEC_o =
{
  ...
  )
  {
    ...
    const trash_s = doc_s === C_o.DOCS_s ?
      ''
      :
      `<${C_o.CLOSE_ICON_TAG_s} class="close_icon" data-doc_s="${doc_s}"><svg class="svg_icon"><use href="#icon_close"></use></svg></${C_o.CLOSE_ICON_TAG_s}>`
    return `<button data-doc_s="${doc_s}">${head_s}${trash_s}</button><article data-doc_s="${doc_s}" class="retract">${main_s}</article>`
  }
,
...
module.exports =
{
  ...
  {
    const doc_s = permalink_s.slice( permalink_s.lastIndexOf( '/' ) + 1, -5 )  //: '.html'.length
    const section_s = `<section id="section_${doc_s}" data-doc_n="${doc_n}" class="invisible">`
    const output_s  = SEC_o.content__v( content_s, doc_s )
